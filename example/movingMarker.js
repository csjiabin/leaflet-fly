...
(function () {
  ...
  L.Marker.include({
    ...
    //设置旋转角度
    setRotationAngle(px1, py1, px2, py2) {
      let x = px2 - px1;
      let y = py2 - py1;
...
const MovingMarker = L.Marker.extend({
  ...
  initialize(latlngs, durations, options) {
...
L.movingMarker = (latlngs, duration, options) => {
  return MovingMarker(latlngs, duration, options);
};
